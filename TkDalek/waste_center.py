...
He is."""
	...
	def rps(self, story_tag, action_tag):
		...
		rps_lbl = Label(action_tag, text=rps_dirs, height=2)
		rps_lbl.config(bg='orange', fg='navy')
		rps_lbl.pack(side=TOP)

		# create radio buttons to answer the question
		rps_var = StringVar()
		for key in self.moves:
			Radiobutton(action_tag, text=key, variable=rps_var, value=key, bg='orange').pack(side=TOP)

		rps_but = Button(action_tag, text='THROW')
		rps_but.config(command=(lambda: self.check_throw(rps_var.get(), story_tag, action_tag)))
		rps_but.pack(side=TOP, fill=X)
